...
// FileRequester — NSOpenPanel pop-up configured for a single FILE
// pick (no directories), returns the chosen path as an AmLang String,
// NULL on cancel. Mirror of FolderRequester.c with the choose-files /
// choose-directories flags swapped. All Cocoa calls go through the
// Objective-C runtime so this stays a plain `.c` file.
...
function_result Am_Ui_FileRequester_requestFile_0(aobject *title, aobject *initialPath, aobject *parent)
{
    ...
    // File mode: choose files, not directories.
    ...
    if (initial_c != NULL && initial_c[0] != '\0' && NSURL != NULL) {
        ...
        // isDirectory:1 — the seed URL points at a folder to open in.
